...
- **既存の spec と重複するテストを追加していない**か。新しく足す example / `context` について、既存 spec 全体を見て確認する。重複かどうかは `context` の文言や実行条件の違いではなく、共通観点（`common.md`）の Testing の基準で判定する。重複があれば追加せず、既存のテストに集約する。
